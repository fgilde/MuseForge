...
The clip plays for its full length (the bracketed context shows each window's duration as "~Ns each", or the total Duration for a single window). Give it enough dialogue and continuous action to last that long — under-writing leaves "dead air" (characters idly looking at each other) — but never write more dialogue than fits at about 2 words per second (a hard ceiling).
