...
def score_audio(filename, checkpoint, melody_only, callback, abort_fn):
    ...
    def report(title, completed, total, force=False):
        ...
        if callback is not None and (force or now - last_update >= 1 / 3):
            last_update = now
            callback(step_idx=max(0, completed - 1), override_num_inference_steps=total, denoising_extra=title, progress_unit=progress.unit + "s")
            if abort_fn():
                raise InterruptedError("SheetSage2 scoring interrupted")
    ...
    try:
        ...
        if abort_fn():
            raise InterruptedError("SheetSage2 loading interrupted")
        model.eval().requires_grad_(False)
        model._offload_hooks = ["encode", "decode"]
        manager = offload.profile({"text_encoder": model}, profile_no=3, budgets={"text_encoder": 0}, pinnedMemory=False, quantizeTransformer=False, convertWeightsFloatTo=None, verboseLevel=1)
