# Adapted for Maestro from Wan2GP 1e1dd2757f24923f008593d9d4ec09062234be20.
# License: ../LICENSES/WanGP-Community-2.0.txt
[CmdletBinding()]
param(
...
if (-not $MaestroAppRoot) {
    $MaestroAppRoot = Split-Path -Parent $PSScriptRoot
}
$MaestroAppRoot = (Resolve-Path -LiteralPath $MaestroAppRoot).Path
if (-not (Test-Path -LiteralPath (Join-Path $MaestroAppRoot "wgp.py") -PathType Leaf)) {
...
function New-InstallItem([string]$Source, [string]$RelativeDestination, [string]$ExpectedHash = "") {
    ...
    return [pscustomobject]@{ Source = $Source; Destination = Join-Path $MaestroAppRoot (Join-Path "dlss5" $RelativeDestination); Sha256 = $ExpectedHash }
}
...
try {
    ...
    if ($conflicts.Count -and -not $Force) {
        ...
        throw "No files were changed. Stop Maestro and rerun with -Force to back up and replace these files."
    }
    ...
    Write-Host "DLSS 5 components are installed in '$MaestroAppRoot\dlss5'. Restart Maestro to refresh the available modes." -ForegroundColor Cyan
}
finally {
