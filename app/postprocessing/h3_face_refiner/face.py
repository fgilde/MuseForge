...
def _select_tracks(frames, detections, face_count=1, identity_threshold=0.28, reference_images=None,
                   ...
                   abort_callback=None, progress_callback=None, reference_threshold=None, reference_margin=0.0):
    app = None
    shot_boundaries = _shot_boundaries(frames)
    if shot_boundaries:
    ...
    for similarity, reference_index, track_index in sorted(matches, reverse=True):
        if similarity < float(identity_threshold if reference_threshold is None else reference_threshold):
            continue
        if reference_index in assigned_references or track_index in assigned_tracks:
            continue
        # Maestro uses a stricter threshold for optional saved-character
        # replacement than for following the same face through a video.
        # Ambiguous candidates should preserve the source identity.
        if any((other_ref == reference_index or other_track == track_index)
               and (other_ref, other_track) != (reference_index, track_index)
               and similarity - other_similarity < float(reference_margin)
               for other_similarity, other_ref, other_track in matches):
            continue
