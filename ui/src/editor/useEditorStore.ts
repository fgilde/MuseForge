...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  beginAIRoundTrip: async (tool, returnMode) => {
    ...
    if (existingRoundTrip && !['completed', 'failed'].includes(existingRoundTrip.status)) {
      set({ error: 'Finish or cancel the current MuseForge AI round trip before starting another.' })
      return
    }
    const project = get().project
    ...
    if (!project || !itemId || !located || located.track.type !== 'video' || asset?.type !== 'video') {
      set({ error: 'Select a video clip before sending it to MuseForge AI.' })
      return
    }
    const saved = await get().saveProject()
    if (!saved) return
  ...
  completeAIRoundTrip: async outputFile => {
    ...
    try {
      ...
      const failed: EditorAIRoundTrip = {
        ...pending,
        status: 'failed',
        error: error instanceof Error ? error.message : 'MuseForge AI result could not return to Editor.',
      }
...
useStore.subscribe(state => {
  ...
  if (pending.claimedJobId !== candidate.id || pending.status !== nextStatus) {
    const updated: EditorAIRoundTrip = {
      ...
      ...(nextStatus === 'failed' ? { error: candidate.error || candidate.message || 'MuseForge AI generation failed.' } : {}),
    }
