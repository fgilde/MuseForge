...
export interface MaestroNotificationEvent {
  ...
  system?: boolean
  /** Keep the in-app toast but suppress MuseForge's device chime. */
  sound?: boolean
...
export function updateDeviceNotificationPreferences(
  ...
  try {
    ...
    // The preference still applies for the current MuseForge session.
  }
...
export function getBrowserNotificationAvailability(): BrowserNotificationAvailability {
  ...
  if (ios && !standalone) {
    return {
      ...
      standalone,
      serviceWorker,
      reason: 'On iPhone and iPad, open MuseForge from an installed Home Screen app—not a Safari or Chrome tab.',
    }
  }
  ...
  if (typeof window === 'undefined' || !window.isSecureContext) {
    return {
      ...
      standalone,
      serviceWorker,
      reason: 'System notifications require HTTPS. A phone opening MuseForge through a local http:// address is not a secure browser context.',
    }
  }
  ...
  if (!notificationApi || !serviceWorker) {
    return {
      ...
      reason: !serviceWorker
        ...
        : 'This browser does not expose notification permission to MuseForge.',
    }
  }
...
function installVisibilityListener() {
  ...
  originalTitle = document.title || 'MuseForge'
...
export async function enableBackgroundPush(
  ...
  if (!host.supported) {
    throw new Error(host.reason || 'Background Web Push is unavailable on this MuseForge host.')
  }
  const registration = await readyServiceWorker()
...
export async function testBrowserNotification(): Promise<boolean> {
  ...
  const event: MaestroNotificationEvent = {
    ...
    category: 'test',
    title: 'MuseForge notifications are ready',
