  ...
  return (
    <section className="space-y-2">
            ...
            {characterFormOpen && (
              <div className="rounded-md border border-border bg-bg-primary p-2 space-y-1.5">
                ...
                <p className="text-[8px] leading-relaxed text-text-muted">
                  Videos remain saved at full length. For each run MuseForge makes H3-ready cached copies: 2–15 seconds each and 15 seconds total (three 10s clips become 5s each).
                </p>
                <button
                  type="button"
                  disabled={disabled || savingCharacter}
      ...
      {references.filter(reference => reference.type === 'video').reduce((sum, reference) => sum + (Number(reference.duration_seconds) || 0), 0) > 15 && (
        <p className="text-[8px] leading-relaxed text-text-muted">
          These video references exceed H3's 15-second combined limit. MuseForge will balance cached trimmed copies across them; your originals and saved characters remain unchanged.
        </p>
      )}
